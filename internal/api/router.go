...
func NewApp(db *pgxpool.Pool, rdb *cache.Cache, rawRedis *redis.Client) *fiber.App {
	...
	// Fila assíncrona de camuflagem de vídeo
	if videoJobsH != nil {
		dash.Post("/shield/videocamo/jobs", videoJobsH.Enqueue)
		dash.Get("/shield/videocamo/jobs", videoJobsH.List)
		dash.Get("/shield/videocamo/jobs/:id/download", videoJobsH.Download)
		dash.Delete("/shield/videocamo/jobs/:id", videoJobsH.Delete)
	}
